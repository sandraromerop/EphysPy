...
for ib in [2]:#range(2,len(brain_reg_interest)):
    for ic in [0]:#range(len(categories)):
        icat = categories[ic]
        brain_reg  = brain_reg_interest[ib]
        ...
        for isess in range(3,len(n_sessions)):
            # isess = 1
            ...
            for (id_neuron_this_sess,id_count) in zip(id_this,np.arange(n_neurons_this_sess)):
                ...
                if id_count==0:
                    ...
                    input_bins_trials =  np.zeros((n_trials,psth_length+1,2))
                    input_trials_sep =  np.zeros((n_trials,psth_length+1,5))
                ...
                psth_zsc_full[:,id_count] = (ps_full-np.nanmean(ps_full))/np.nanstd(ps_full) 
                sp_bins_zsc[:,id_count] = (sp_full- np.nanmean(sp_full))/np.nanstd(sp_full)
            ...
            for (id_this_neuron,icount) in zip(this_neuron_ids,range(len(id_this))):
                ...
                kn = 'spc_neuron_' + str(id_this_neuron[0])
            ...
            neurons = list(xdf.filter(like='spc_neuron_'))
            xdf_neurons = xdf[neurons]
            xdf[xdf_neurons.add_suffix("_spc_zscore").columns] = ((xdf_neurons - xdf_neurons.mean())/xdf_neurons.std())
            neurons_zscore_spc = list(xdf.filter(like='_spc_zscore'))
            ...
            cumcount_mean = X_drift.groupby(["bins_cue_type", "cumcount"]).transform("mean")[neurons_zscore] # mean across trials at each time step
            drift = (X_drift[neurons_zscore] - cumcount_mean) # activity with no within-trial variability
            drift["bins_trial_numb"] = X_drift["bins_trial_numb"]
            ...
            plot_drift_set(X_new, neurons_zscore)

            neurons_zscore_res = [s + "_res" for s in neurons_zscore_spc]
            y_res = pd.DataFrame(columns=neurons_zscore_res)
            cumcount_mean = X_drift.groupby(["bins_cue_type", "cumcount"]).transform("mean")[neurons_zscore_spc] # mean across trials at each time step
            drift = (X_drift[neurons_zscore_spc] - cumcount_mean) # activity with no within-trial variability
            drift["bins_trial_numb"] = X_drift["bins_trial_numb"]
            ...
            for neuron in neurons_zscore_res:
                X_new[neuron] = pd.to_numeric(X_new[neuron])
            plot_drift_set(X_new, neurons_zscore_spc)
